...
DATA_DIR = "data"
BOT_GROUPS_FILE = os.path.join(DATA_DIR, "bot_groups.json")
os.makedirs(DATA_DIR, exist_ok=True)
...
def load_bot_groups():
    ...
    if os.path.exists(BOT_GROUPS_FILE):
        try:
            with open(BOT_GROUPS_FILE, "r") as f:
                bot_groups = json.load(f)
            logger.info(f"Loaded {len(bot_groups)} groups from {BOT_GROUPS_FILE}")
        except:
            logger.error("Failed to load bot_groups")
    else:
        logger.info("No bot_groups.json found, starting empty")

def save_bot_groups():
    try:
        with open(BOT_GROUPS_FILE, "w") as f:
            json.dump(bot_groups, f)
        logger.info(f"Saved {len(bot_groups)} groups")
    except Exception as e:
        logger.error(f"Save bot_groups error: {e}")
...
async def on_bot_chat_member_update(event: types.ChatMemberUpdated):
    ...
    if new_status == "member" or new_status == "administrator":
        ...
        bot_groups[str(chat.id)] = True
        logger.info(f"Bot added to group: {chat.id} ({chat.title or 'no title'}) | Groups: {len(bot_groups)}")
        save_bot_groups()
        ...
        if str(chat.id) in bot_groups:
            del bot_groups[str(chat.id)]
        logger.info(f"Bot removed from group: {chat.id} | Groups: {len(bot_groups)}")
        save_bot_groups()
