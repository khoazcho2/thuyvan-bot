...
async def ping_public(message: Message):
    logger.info(f"PING PUBLIC ENTRY chat={message.chat.id} user={message.from_user.id}")
    try:
        await message.answer(f"✅ Bot hoạt động!\nChat ID: {message.chat.id}\nType: {message.chat.type}")
    except Exception as e:
        logger.error(f"PING PUBLIC ERROR chat={message.chat.id}: {e}")

@dp.message(Command("help"))
async def help_public(message: Message):
    logger.info(f"HELP PUBLIC ENTRY chat={message.chat.id}")
    help_text = """📋 <b>DANH SÁCH LỆNH BOT</b>

<b>✅ Lệnh công khai (ai cũng dùng được):</b>
/start - Khởi động bot
/ping - Kiểm tra bot hoạt động  
/test - Test bot (debug)
/help - Xem danh sách lệnh này

<b>🔐 Lệnh Admin (bot phải là admin group):</b>
/ban, /unban, /mute [thời gian] - Quản lý user
/locklink, /unlocklink - Chặn/mở link
/lock18*, /unlock18* - Chặn nội dung 18+
/schedule, /unschedule - Tin nhắn định kỳ
/canhbao*, /xemcanhbao - Hệ thống cảnh báo

<b>👑 Lệnh Owner (chỉ chủ bot):</b>
/banglobal, /unbanglobal, /dsglobal - Ban toàn cầu
/globalgroup [text] - Gửi tin đến tất cả groups

<i>Thêm bot làm ADMIN group để dùng lệnh đầy đủ!</i>"""
    try:
        await message.answer(help_text)
    except Exception as e:
        logger.error(f"HELP PUBLIC ERROR chat={message.chat.id}: {e}")
